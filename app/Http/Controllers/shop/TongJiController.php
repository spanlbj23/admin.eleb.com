...
//        $shop_id=O
        ...
        $sql = "select date(created_at) date,count(*) total from orders where created_at >='{$time_start}' and created_at <='{$time_end}' group by date(created_at)";
        $rows = DB::select($sql);
//        dd($rows);
        //构造一个七天统计格式
        $result = [];
...
//        $shop_id=O
        ...
        $sql = "select date_format(created_at,'%Y-%m') date,count(*) total from orders where created_at >='{$time_start}' and created_at <='{$time_end}' group by date";

//        $sql = "select date_format(created_at,'%Y-%m') date,count(*) total from orders where created_at >='2018-09-01 00:00:00' and created_at <='2018-11-30 23:59:59' group by date";
        $rows = DB::select($sql);
//        dd($rows);
        //构造一个三个月统计格式
        $result = [];
